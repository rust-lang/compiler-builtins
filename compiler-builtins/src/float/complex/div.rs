...
fn div<F: Float>(mut a: F, mut b: F, mut c: F, mut d: F) -> Complex<F>
where
    u32: CastInto<F::Int>,
{
...
intrinsics! {
    ...
    // On PowerPC `__divtc3` is for double-double `long double` rather than IEEE `f128`.
    #[ppc_name = __rust_divkc3]
    #[cfg(f128_enabled)]
