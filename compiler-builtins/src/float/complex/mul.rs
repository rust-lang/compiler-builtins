...
fn mul<F: Float>(mut a: F, mut b: F, mut c: F, mut d: F) -> Complex<F> {
    let ac = a * c;
    let bd = b * d;
    let ad = a * d;
    let bc = b * c;
...
intrinsics! {
    ...
    // On PowerPC `__multc3` is for double-double `long double` rather than IEEE `f128`.
    #[ppc_name = __rust_mulkc3]
    #[cfg(f128_enabled)]
