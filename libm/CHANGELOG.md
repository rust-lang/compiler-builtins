...
## [0.2.17](https://github.com/rust-lang/compiler-builtins/compare/libm-v0.2.16...libm-v0.2.17) - 2026-09-10

### Other

- Fix thumb_1 cfg to include all "thumb but not thumbv2" targets.
- Add a disabled assertion for thumb mode config
- Rename `thumb_1` to `thumb1_only`
- Make `assert_biteq` usable outside of `libm`
- Add a quieted signaling NaN constant
- support hardware sqrtf/sqrt on Arm32 and Thumb targets
- Remove an MSRV hack that is no longer needed
- implement `Complex` mul and div
- Add comments about why miri is configured out
- Expose `hypotf64` in `relaxed`
- Rename the `approx` module to `relaxed`
- generator for varying specific bits
- Clean up lints
- Rename the `cfg_select` polyfill to `cfg_select_nofmt`
- Don't export `cfg_select`
- update comment for using workspace-inherited dependency
- test Miri on libm
